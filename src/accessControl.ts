...
export function isLogin(): boolean {
  ...
  if (!token || !expireTimeStr) {
    ...
    console.log("!TOKEN OR !EXPIRETIMESTR")
    return false;
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    authAgree(code)
      .then(res => {
        ...
        // await storage.setItem(`${APP_KEY}_token`, token);
        ...
        // );
        localStorage.setItem(`${APP_KEY}_expire_time`, (Number(expireTime) * 1000 + Date.now() - 300000).toString(),)
      })
      .catch(error => {
        console.log(error);
        reject(false);
